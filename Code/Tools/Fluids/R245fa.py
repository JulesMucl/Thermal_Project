...
pressures = np.linspace(PropsSI('P_MIN', 'R245fa'), PropsSI('P_CRITICAL', 'R245fa'), 500)
print("P_min ===",PropsSI('P_MIN', 'R245fa')/1e5,'bar')
print("P_critical ===",PropsSI('P_CRITICAL', 'R245fa')/1e5,'bar')
...
for P in pressures:
    T = PropsSI('T', 'P', P, 'Q', 0, 'R245fa')  # Température de saturation (Q=0 pour liquide)
    S_f = PropsSI('S', 'P', P, 'Q', 0, 'R245fa')  # Entropie du liquide saturé
    S_g = PropsSI('S', 'P', P, 'Q', 1, 'R245fa')  # Entropie de la vapeur saturée
...
for P in isobare_pressures:
    ...
    for Q in np.linspace(0, 1, 100):  # Parcours de la qualité de 0 à 1
        T = PropsSI('T', 'P', P, 'Q', Q, 'R245fa')
        S = PropsSI('S', 'P', P, 'Q', Q, 'R245fa')
...
plt.title('Diagramme T-s du R245fa avec Isobares')
plt.legend()
plt.grid(True)
plt.show()
