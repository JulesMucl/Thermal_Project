...
pressures = np.linspace(PropsSI('P_MIN', 'IsoButane'), PropsSI('P_CRITICAL', 'IsoButane'), 500)
print("P_min ===",PropsSI('P_MIN', 'IsoButane')/1e5,'bar')
print("P_critical ===",PropsSI('P_CRITICAL', 'IsoButane')/1e5,'bar')
...
for P in pressures:
    T = PropsSI('T', 'P', P, 'Q', 0, 'IsoButane')  # Température de saturation (Q=0 pour liquide)
    S_f = PropsSI('S', 'P', P, 'Q', 0, 'IsoButane')  # Entropie du liquide saturé
    S_g = PropsSI('S', 'P', P, 'Q', 1, 'IsoButane')  # Entropie de la vapeur saturée
...
isobare_pressures = np.linspace(pressures[0], pressures[-1], 8)#[1e5, 2e5, 5e5, 1e6, 2e6]  # en Pascal
for P in isobare_pressures:
    S_vals = []
    T_vals = []
    for Q in np.linspace(0, 1, 100):  # Parcours de la qualité de 0 à 1
        T = PropsSI('T', 'P', P, 'Q', Q, 'IsoButane')
        S = PropsSI('S', 'P', P, 'Q', Q, 'IsoButane')
...
plt.title('Diagramme T-s de l IsoButane avec Isobares')
plt.legend()
plt.grid(True)
plt.show()
